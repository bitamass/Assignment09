...
# Bmassoudi, 2022-Dec-10, Modified the File to add tracks functionality
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name, lst_Inventory) -> None:
        ...
        try:
            objFile = open(file_name[0],'w')
            for item in lst_Inventory:
                objList = item.get_record()
                objFile.write(objList + '\n')
            objFile.close()
        # TO DONE add code to save track data to file
            objFile1 = open(file_name[1],'w')
            for item in lst_Inventory:
                objTrack = item.cd_tracks
                itemCD = item.cd_id
                for item1 in objTrack:
                    objFile1.write(itemCD, objTrack.get_record())
            objFile1.close()
    ...
    def load_inventory(file_name: list) -> list:
        ...
        # TO DOne modify method to accept a list of file names
        try:
            objFile = open(file_name[0], 'r')
            for row in objFile:
                rowData = row.strip().split(',')
                cd_info = DC.CD(rowData[0], rowData[1], rowData[2])
                lst_Inventory.append(cd_info)
            objFile.close()

         # TO DOne add code to load track data
            objFile1 = open(file_name[1],'r')
            for row1 in objFile1:
                rowData1 = row1.strip().split(',')
                cdObj = PC.DataProcessor().select_cd(lst_Inventory, rowData1[0])
                cdObj.add_track(DC.Track(rowData1[1], rowData1[2], rowData1[3]))
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
